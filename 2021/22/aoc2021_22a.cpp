...
bool intersect(const Cuboid& a, const Cuboid& b, Cuboid& c)
{
	if (!intersects(a, b))
		return false;
	...
	c.front  = std::max(a.front,  b.front);
	c.back   = std::min(a.back,   b.back);
...
int main(int argc, const char* argv[])
{
	...
	const Cuboid init_region { -50, 51, -50, 51, -50, 51 }; 

	__int64 count = 0;

	Cuboid intersection;
	for (const auto& cuboid : cuboids)
		if (intersect(cuboid, init_region, intersection))
